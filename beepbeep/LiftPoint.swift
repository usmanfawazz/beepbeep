...
struct LiftPoint: Codable, Identifiable {
    ...
    enum CodingKeys: String, CodingKey {    // encode CLLocationCoordinate2D manually
        case latitude, longitude, altitude, timestamp, id
    }

    init(coordinate: CLLocationCoordinate2D, altitude: Double, timestamp: Date = Date()) { //initializer for creating a liftpoint
